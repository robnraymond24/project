...
def chart_user_input(index):
    ...
    chart_color = option_1.selectbox("Select Color for Charts", ["Red", "Green", "Yellow", "Orange", "Blue", "Purple"],
                                     key=index)
...
def pie_options(index):
    ...
    explode_amount_used = []
    if explode_amount == 0:
        explode_amount_used = [0] * 11
    elif explode_amount == 0.1:
        explode_amount_used = [0.1] * 11
    elif explode_amount == 0.2:
        explode_amount_used = [0.2] * 11
    elif explode_amount == 0.3:
        explode_amount_used = [0.3] * 11
...
def load_charts():
    ...
    years = []
    for x in data["Last Known Eruption"]:
        if "BCE" in x:
            x = -int(x.split(" ")[0])
        elif "CE" in x:
            x = int(x.split(" ")[0])
...
def load_maps():
    ...
    sea_level = st.multiselect("Select data to display on the map", options)
    volcanoes_list = ""
